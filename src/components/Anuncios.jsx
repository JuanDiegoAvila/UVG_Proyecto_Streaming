...
function Anuncios({ cantidad, setAnuncios }) {
    ...
    useEffect(async () => {
        ...
        const resp = await fetch('https://apistreaming.juanangelcarrera.xyz/streaming/anuncios')
    ...
    const vistaAnuncio = async (anuncio) => {
        ...
        const resp = await fetch('https://apistreaming.juanangelcarrera.xyz/streaming/adseen', options)
            .then((response) => { return response.json() }
            ).then((responseInJSON) => { return responseInJSON })
    }
